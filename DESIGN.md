...
같은 터미널을 밤에 본 것이다. 사인밴드와 데이터 보드는 **값이 바뀌지 않는다** — 공항 안내판은 정오와 자정에 같은 색으로 읽힌다. 움직이는 것은 그라운드·시트·잉크뿐이다. iOS는 `Theme.swift`의 `OMP.adaptive(light:dark:)`가, 웹은 `prefers-color-scheme`이 담당한다.

- **콘크리트** #EDEDE9 → **#161614** (야간 아스팔트 그라운드)
- **시트** #FFFFFF → **#212120** (슬레이트 시트)
- **시트 인셋** #F4F4F0 → **#2B2B29**
- **보더** #D9D9D2 → **#383833**
- **잉크** #151513 → **#F2F2EE** / **잉크 뮤트** #5D5D57 → **#A5A59C**
- **제트블랙 보드** #0E0E0D → **#0B0B0A** (그라운드보다 계속 어두워야 보드로 읽힌다)
- **패널 레이즈드** #242422 → **#1A1A18**
- **시맨틱**은 다크에서 온패널 변형과 같은 밝기대로 올린다: 녹 → #7BD8A0, 적 → #FF7A6B, 앰버 → #E8B23A
- **옐로 3종·패널 라인·온패널**: 불변

### Named Rules
**The Yellow-Is-Wayfinding Rule.** 옐로는 목표와 안내(다음 결정 지점)에만 쓴다. 상태 표시는 신호등 3색, 장식은 존재하지 않는다. 옐로가 흔해지는 순간 사인 시스템이 죽는다.

**The Fixed-Sign Rule.** 사인밴드(옐로)와 데이터 보드(제트블랙)는 조명이 바뀌어도 같은 값을 유지한다. 다크 모드는 배경 재질과 잉크만 뒤집는 조명 변경이지, 두 번째 디자인이 아니다. 여기서 "다크 네온 피트니스 문법 거부"는 그대로 유효하다 — 야간에도 옐로는 목표에만, 나머지는 무채색이다.

**The Paired-Token Rule.** 표면 토큰과 그 위의 잉크 토큰은 항상 같이 움직인다. 고정 배경 위에 시스템 시맨틱 색(SwiftUI `.primary`/`.secondary`, CSS `色` 기본값)을 얹지 않는다 — 배경만 고정이고 글자만 뒤집히면 흰 시트 위 흰 글자가 된다. 보드·사인밴드처럼 고정 표면 위 텍스트는 반드시 고정 잉크(온패널/옐로 잉크)를 명시한다.

**The Prominent-Button Rule.** 주 액션 버튼은 잉크/시트 쌍의 반전(`ompProminent`)으로 그린다. `.borderedProminent`는 액센트로 채우고 라벨을 항상 흰색으로 그려서, 다크에서 액센트가 밝아지는 순간 라벨이 사라진다.
